...
class Polyline(object):
    def __init__(self, points=None, points_string=None):
        if points is None and points_string is None:
            ...
            self.points = [Point(map(float, p.split(","))) for p in pn]
            # sets bounds
            self.top, self.right, self.bottom, self.left = self.set_bounds()
...
class Polygons(object):
    ...
    def __init__(self, filename):
        ...
        self.polygons = [Polygon(p.getAttribute("points")) for p in poly if p.getAttribute("display") != "none" and p.getAttribute("class") != "hole"]
        self.holes = [p for p in poly if p.getAttribute("display") != "none" and p.getAttribute("class") == "hole"]
    ...
    def connect(self):
        ...
        for i in range(1, len(self.sorted)):
            print i 
            pl = self.sorted[i]
            polylines.append(Polyline(points=pl.closest(points)))
    ...
    def write_segments(self, x0, x, output):
        ...
        for segment in segments:
            ...
            for pl in self.holes:
                node = doc.createElement("polyline")
                node.setAttribute("stroke","#ff0000")
                node.setAttribute("fill", "none")
                node.setAttribute("stroke-width", "0.1")
                node.setAttribute("points", pl.getAttribute("points"))
                g.appendChild(node)
            for pl in segment:
