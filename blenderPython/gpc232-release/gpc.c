...
gpc_polygon *gpc_read_all_polygon(FILE *fp)
{
  int c = 0;
  int v, i;
  int num_polygons;
  gpc_polygon* acc;
  MALLOC(acc, sizeof(gpc_polygon), "allocating accumulated gpc", gpc_polygon);

  acc->num_contours = 1;

  fscanf(fp, "%d", &(num_polygons));
  MALLOC(acc->hole, acc->num_contours * sizeof(int),
         "hole flag array creation", int);
  MALLOC(acc->contour, acc->num_contours
  ...
  for (i= 1; i < num_polygons; i++)
  {
    ...
    gpc_polygon cur;
    gpc_polygon* res;
    MALLOC(res, sizeof(gpc_polygon), "allocating output gpc", gpc_polygon);

    cur.num_contours = 1;

    MALLOC(cur.hole, cur.num_contours * sizeof(int),
           "hole flag array creation", int);
    MALLOC(cur.contour, cur.num_contours
           * sizeof(gpc_vertex_list), "contour creation", gpc_vertex_list);

    fscanf(fp, "%d", &(cur.contour[c].num_vertices));
    ...
    printf("DEBUG: cur %d\n",i);
    gpc_print_polygon(&cur);
    gpc_polygon_clip(GPC_UNION, acc, &cur, res);
    // printf("DEBUG: acc\n");
    // gpc_print_polygon(acc);
    // printf("DEBUG: res\n");
    // gpc_print_polygon(res);

    gpc_free_polygon(&cur);
